...
const CollegeStudents = () => {
  ...
  const fetchUsersByCollege = async () => {
    ...
    try {
      const response = await fetch(`/api/user/getUsersByCollege/${collegeName}?startIndex=${startIndex}&status=${statusFilter}&course=${selectedDegreeProgram}`);
  ...
  const handleExport = async () => {
    try {
      ...
      // Generate file name
      const fileName = `${collegeName.replace(/\s+/g, '_')}_Online.xlsx`;
