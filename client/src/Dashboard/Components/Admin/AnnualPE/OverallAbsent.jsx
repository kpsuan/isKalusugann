...
const OverallAbsent = () => {
  ...
  const fetchUsersAbsent = async () => {
    const startIndex = (currentPage - 1) * limit;

    try {
        const response = await fetch(
          `/api/user/overallabsent?startIndex=${startIndex}&limit=${limit}&status=${statusFilter}&course=${selectedDegreeProgram}${filter ? `&filter=${filter}` : ''}`
        );
    ...
    <div className="dashboard my-flex">
      <div className="dashboardContainer my-flex">
        ...
        <div className="mainContent m-0 p-0">
          <div className=" bg-gradient-to-r from-red-700 to-red-500 rounded-lg border border-gray-200 p-10 w-full">
            <div className="text-5xl font-bold  text-white mb-4">Overall Absent</div>
