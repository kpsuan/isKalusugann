...
const OverallArrived = () => {
  ...
  const fetchUsersPresent = async () => {
    const startIndex = (currentPage - 1) * limit;

    try {
        const response = await fetch(
          `/api/user/overallpresent?startIndex=${startIndex}&limit=${limit}&status=${statusFilter}&course=${selectedDegreeProgram}${filter ? `&filter=${filter}` : ''}`
        );
      ...
      <div className="dashboardContainer my-flex">
        ...
        <div className="mainContent m-0 p-0">
          <div className=" bg-gradient-to-r from-blue-700 to-cyan-500 rounded-lg border border-gray-200 p-10 w-full">
            <div className="text-5xl font-bold  text-white mb-4">Overall Arrived</div>
            <p className="font-light text-lg my-8 text-white">
             List of students who were able to attend to their 
