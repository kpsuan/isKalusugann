...
const YesterdayPresent = () => {
  ...
  const fetchUsersPresentYesterday = async () => {
    const startIndex = (currentPage - 1) * limit;

    try {
        const response = await fetch(
          `/api/user/yesterdaypresent?startIndex=${startIndex}&limit=${limit}&status=${statusFilter}&course=${selectedDegreeProgram}${filter ? `&filter=${filter}` : ''}`
        );
...
useEffect(() => {
    fetchUsersPresentYesterday();
}, [statusFilter, selectedDegreeProgram, currentPage, filter]);
...
const yesterdate = yesterday.toLocaleDateString('en-US', {
      ...
      <div className="dashboardContainer my-flex">
        ...
        <div className="mainContent m-0 p-0">
          <div className=" bg-gradient-to-r from-blue-700 to-cyan-500 rounded-lg border border-gray-200 p-10 w-full">
            <div className="text-4xl font-bold  text-white mb-5 ">{yesterdate} </div>
            <p className="text-3xl font-light  text-white ">End of Day Report</p>
            <p className="font-light text-lg my-4 text-white">
             List of students who were able to attend and not attend to their 
...
export default YesterdayPresent;
