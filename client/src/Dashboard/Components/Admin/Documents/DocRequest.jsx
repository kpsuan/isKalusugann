...
const DocRequest = () => {
 ...
 const handleActionSubmit = async () => {
  try {
    ...
    const response = await fetch(`/api/docrequest/updateStatus2`, {
      method: "PUT",
...
const handleEmailUser = async (request) => {
  ...
  useEffect(() => {
    const fetchRequestedDocument = async () => {
      setIsLoading(true);
      try {
        const res = await fetch('/api/docrequest/getRequestHistoryDoctor');
        if (!res.ok) {
      ...
      <React.Fragment key={request._id}>
        <Table.Row className="bg-white hover:bg-gray-50">
          ...
          <Table.Cell>
            ...
            <Tooltip content="Mark As Done" className='text-black'>
                <Button 
                  size="sm" 
                  color="green" 
                  className="p-2"
              ...
              <Tooltip content="Delete Request" className='text-black' >
                <Button 
                  size="sm" 
                  color="red" 
                  className="p-2"
