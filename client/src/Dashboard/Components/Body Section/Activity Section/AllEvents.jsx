        ...
        events.map((events, index) => (
          <div key={index} className="flex flex-col md:flex-row border border-gray-300 mb-5 rounded-lg shadow bg-gray-50">
            <div className="relative w-full md:w-1/3 overflow-hidden ">
              <img 
                src={events.image || "https://via.placeholder.com/150"} 
                alt={events.title} 
                ...
                onClick={() => handleImageClick(events.image)}
              />
            </div>
            <div className="w-full md:w-2/3 p-4">
              <Link to={`/post/${events.slug}`} target="_blank" rel="noopener noreferrer">
                <Card className="max-w-full p-5">
                  <div className="text-center w-24">
                    {getBadge(events.category)}
                  </div>
                  <h5 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                    {events.title}
                  </h5>
                  <span className="text-sm text-gray-500 mb-2">
                    Posted on: {new Date(events.updatedAt).toLocaleDateString('en-US', {
                      month: 'long',
                      day: 'numeric',
                      year: 'numeric'
                    })}
